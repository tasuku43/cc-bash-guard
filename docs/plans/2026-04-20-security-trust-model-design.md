...
This plan defines the publication-time security stance for `cc-bash-guard`.
...
rewrite behavior. Because `cc-bash-guard` runs directly in the command path before
...
- add `cc-bash-guard version --build-info`
