...
This document defines the supported stdin payloads for `cmdguard eval` v1 and
the normalization step that turns them into a command-string evaluation request.

## 2. Canonical Execution Request

Internally, `cmdguard eval` should normalize supported inputs into a canonical
