...
This document defines the supported stdin payloads for `cc-bash-guard hook` and
the normalization step that turns them into a command-string evaluation request.

## 2. Canonical Execution Request

Internally, `cc-bash-guard hook` should normalize supported inputs into a canonical
