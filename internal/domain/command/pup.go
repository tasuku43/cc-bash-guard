...
func buildPupSemantic(actionPath []string, options []Option) *PupSemantic {
	...
	if len(actionPath) > 0 { semantic.Area = actionPath[0] }
	if len(actionPath) > 1 { semantic.Verb = actionPath[1] }
	if len(actionPath) > 2 {
...
func pupLastOptionValue(options []Option, names ...string) string {
	for i := len(options)-1; i >= 0; i-- {
		for _, n := range names {
			if options[i].Name == n && options[i].HasValue { return options[i].Value }
		}
	}
	return ""
}
