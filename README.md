...
- Claude Code hook integration via `cmdproxy hook claude`
...
- `--rtk` integration after `cmdproxy` rewrite and permission evaluation
...
- the CLI surface: `hook`, `check`, `doctor`, `verify`, `version`, and `init`
...
For shell-sensitive spot checks, prefer a single quoted command string:
...
`cmdproxy` is currently **Claude Code-first**. The public hook entrypoint,
