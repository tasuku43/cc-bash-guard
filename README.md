...
artifact fingerprint. Claude settings files without Bash `permissions.allow`,
`permissions.ask`, or `permissions.deny` rules do not make the artifact stale.
When the artifact is missing or stale, the hook regenerates it only after the
current effective config passes the same verification checks as
`cc-bash-guard verify`; if verification fails, the hook returns `ask` with a
warning instead of evaluating stale policy. Incompatible artifacts still fail
closed. Run `cc-bash-guard verify` directly when you want to inspect diagnostics
after policy edits, include edits, Claude permission changes, or binary
upgrades.
